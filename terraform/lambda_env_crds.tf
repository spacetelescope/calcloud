module "calcloud_env_crds" {
  source = "terraform-aws-modules/lambda/aws"
  # https://github.com/hashicorp/terraform/issues/17211
  version = "~> 1.43.0"

  function_name = "calcloud-env-crds${local.environment}"
  description   = "updates the crds context in the Batch job definitions"
  # the path is relative to the path inside the lambda env, not in the local filesystem.
  handler       = "crds_handler.lambda_handler"
  runtime       = "python3.6"
  publish       = false
  timeout       = 900
  ...
  source_path = [
    {
      ...
      path = "${path.module}/../lambda/EnvCRDS"
  ...
  lambda_role = "arn:aws:iam::218835028644:role/calcloud-lambda-crds"
  ...
  tags = {
    Name = "calcloud-env-crds${local.environment}"
  }
}

# for the s3 event trigger for crds lambda
resource "aws_lambda_permission" "allow_bucket_crds" {
  statement_id  = "AllowExecutionFromS3Bucket"
  action        = "lambda:InvokeFunction"
  function_name = module.calcloud_env_crds.this_lambda_function_arn
